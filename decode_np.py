...
class Decode(object):
    def __init__(self, obj_threshold, nms_threshold, input_shape, all_classes, graph, iftiny=True):
        ...
        if iftiny:
            ...
            self.output_m = graph.get_tensor_by_name('detector/yolo-v4-tiny/Reshape_4:0')  # medium: (?,2028,6) ->26*26*3=2028
        else:
    ...
    def predict(self, image, shape):
        ...
        if self.iftiny:
            outs1, outs2 = tf.get_default_session().run([self.output_s, self.output_m], feed_dict={self.input: image})
            # numpy后处理
            a1 = np.reshape(outs1, (1, self.input_shape[0]//32, self.input_shape[1]//32, 3, 5+self.num_classes))
            a2 = np.reshape(outs2, (1, self.input_shape[0]//16, self.input_shape[1]//16, 3, 5+self.num_classes))
            boxes, scores, classes = self._yolo_out([a1, a2], shape)
        else:
            outs1, outs2, outs3 = tf.get_default_session().run([self.output_s, self.output_m, self.output_l],
                                                               feed_dict={self.input: image})
            # numpy后处理
            a1 = np.reshape(outs1, (1, self.input_shape[0]//32, self.input_shape[1]//32, 3, 5+self.num_classes))
            a2 = np.reshape(outs2, (1, self.input_shape[0]//16, self.input_shape[1]//16, 3, 5+self.num_classes))
            a3 = np.reshape(outs3, (1, self.input_shape[0]//8, self.input_shape[1]//8, 3, 5+self.num_classes))
