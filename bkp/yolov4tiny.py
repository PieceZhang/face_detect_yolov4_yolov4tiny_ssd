...
if __name__ == '__main__':
    ...
    image_origin2 = cv2.imread('image2.png')
    ...
    with tf.Session(graph=graph) as sess:
        ...
        for idx1, value1 in enumerate(boxes):
            # 解决yolov4tiny输出bbox大小异常
            value1[0] += int((value1[2] - value1[0]) / 3.5)
            value1[2] -= int((value1[2] - value1[0]) / 3.5)
            value1[1] += int((value1[3] - value1[1]) / 3)
            value1[3] -= int((value1[3] - value1[1]) / 3)
            for value2 in value1:
                if value2 > input_shape[0]+30 or value2 < -30:
                    dellist.append(idx1)
                    break
            if value1[2] - value1[0] < 15 or value1[3] - value1[1] < 15:
                dellist.append(idx1)
                continue
