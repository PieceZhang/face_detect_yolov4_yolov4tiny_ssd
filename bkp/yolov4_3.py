...
def decode(conv_output, anchors, stride, num_class):
    ...
    # pred_xywh, pred_conf, pred_prob = sess.run([pred_xywh, pred_conf, pred_prob], feed_dict1)
...
if __name__ == '__main__':
    ...
    """
    参考github例程，只使用中等尺度feature map
    并修改了decode（函数不再返回数值，改为返回tensor，可能更慢）
    """
    with tf.Session(graph=graph) as sess:
        feed_dict1 = {node_in: image1}
        feed_dict2 = {node_in: image2}
        # warm up
        _ = sess.run([output_s, output_m, output_l], feed_dict1)

        timelist.append(time.time())

        output_m= sess.run(output_m, feed_dict2)
        ...
        output = fastnms(pred_xywh_m, pred_score_m, conf_thresh, nms_thresh, keep_top_k, nms_top_k)

        timelist.append(time.time()), print('2',timelist[-1] - timelist[-2])  # time 2

        output = sess.run(output,feed_dict1)
    ...
    if boxes is not None:
        draw(image_origin1, boxes, scores, classes, all_classes, colors)
